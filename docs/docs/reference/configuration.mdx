...
| Variable | Description | Default | Example |
|----------|-------------|---------|----------|
| `CLAUDE_CREDENTIALS` | Base64-encoded Claude credentials (for Docker) | `""` | `<base64 of ~/.claude/.credentials.json>` |

For local development, authenticate directly with `claude setup-token`. For Docker, export credentials from an authenticated machine with `cat ~/.claude/.credentials.json | base64 -w0`.
...
data:
  ...
  claude-credentials: <base64-encoded-credentials>
```
