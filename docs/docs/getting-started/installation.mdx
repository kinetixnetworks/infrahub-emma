...
Emma is an experimental AI-powered assistant for Infrahub that helps you manage infrastructure schemas and data through an intuitive web interface. This fork uses [Claude Code](https://docs.anthropic.com/en/docs/claude-code) by Anthropic for AI features.
...
- **Claude Code CLI**: Required for AI-powered features (Schema Builder, Query Builder, Template Builder)
- **Claude Pro or Max subscription**: Required to authenticate with Claude Code
...
```bash
git clone https://github.com/kinetixnetworks/infrahub-emma.git
cd infrahub-emma
```

2. **Export your Claude credentials** (from a machine where Claude Code is already authenticated):

```bash
cat ~/.claude/.credentials.json | base64 -w0
```

3. **Start Emma with Docker Compose:**

```bash
CLAUDE_CREDENTIALS="<base64-string>" docker compose up --build -d
```

4. **Connect to Infrahub network** (if running Infrahub in containers):
