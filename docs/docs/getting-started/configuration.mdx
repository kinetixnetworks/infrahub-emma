...
| `CLAUDE_AUTH_TOKEN` | Claude Code authentication token (for Docker) | `""` | `<token from claude setup-token>` |
...
For local development, authenticate with `claude setup-token`. For Docker deployments, pass your token via the `CLAUDE_AUTH_TOKEN` environment variable.
