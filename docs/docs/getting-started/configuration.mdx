...
For Claude Code, authenticate locally (not via env var):
...
Emma uses the [Claude Code CLI](https://docs.anthropic.com/en/docs/claude-code) for AI-powered features. Claude Code must be installed and authenticated before using the Schema Builder, Query Builder, or Template Builder.

For local development, authenticate with `claude setup-token`. For Docker deployments, pass your base64-encoded credentials via the `CLAUDE_CREDENTIALS` environment variable.
