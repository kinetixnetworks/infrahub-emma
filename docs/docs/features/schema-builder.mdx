...
The Schema Builder leverages [Claude Code](https://docs.anthropic.com/en/docs/claude-code) by Anthropic to understand infrastructure concepts and translate natural language requirements into properly structured Infrahub schemas. It understands:
...
- **CLI invocation**: Each generation invokes the Claude Code CLI
- **Response time**: Complex descriptions may take longer to process
- **Subscription**: A Claude Pro or Max subscription is required
...
- Ensure Claude Code CLI is installed and authenticated (`claude setup-token`)
